...
# 编译
./mvnw clean package -DskipTests -pl flink-paimon-demo -am
```
...
flink run -c io.sophiadata.flink.paimon.MySqlToPaimonPipeline \
  target/flink-paimon-demo-1.0.0.jar \
...
```bash
# 运行单元测试
./mvnw test -pl flink-paimon-demo -Dtest='MongoTypeMapperTest,MongoDocumentFlattenerTest'

# 运行所有测试
./mvnw test -pl flink-paimon-demo
```
