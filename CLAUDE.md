...
| `cdc-mysql-sync` | 整库同步：`flink-cdc 3.x` → `MySqlCatalog` → JDBC sink，含 `SchemaEvolver` |
| `flink-demo` | 可复用 Flink `TableFunction` 示例 |
...
- **包名**: 主包 `io.sophiadata.flink.*`；`flink-demo` 用 `com.zyzx.realtime.flink.function`
